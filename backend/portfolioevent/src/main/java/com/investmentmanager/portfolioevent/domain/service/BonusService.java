...
public class BonusService implements BonusUseCase {
    ...
    public PortfolioEvent create(CreateBonusCommand command) {
        ...
        int baseQuantity = assetPositionQueryPort.findQuantityAsOfDate(ticker, brokerKey, command.getEventDate())
                .orElseThrow(() -> new IllegalStateException(
                        "Posição base não encontrada para o ticker na data do evento: " + ticker));
        if (baseQuantity <= 0) {
            throw new IllegalStateException("Posição base sem quantidade elegível na data do evento: " + ticker);
        }

        int bonusQuantity = BigDecimal.valueOf(baseQuantity)
                .multiply(BigDecimal.valueOf(ratio.getBonus()))
                .divide(BigDecimal.valueOf(ratio.getBase()), 0, RoundingMode.DOWN)
                .intValueExact();
        if (bonusQuantity <= 0) {
            throw new IllegalStateException("Bonificação não gera quantidade elegível na data do evento");
        }
        ...
        PortfolioEvent bonus = PortfolioEvent.create(
                ...
                sourceReferenceId,
                null,
                PortfolioEventMetadata.bonus(ratio.canonical(), baseQuantity));
