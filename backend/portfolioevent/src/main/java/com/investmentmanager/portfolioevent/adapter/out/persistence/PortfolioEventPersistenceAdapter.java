...
class PortfolioEventPersistenceAdapter implements PortfolioEventRepositoryPort {
    ...
    public List<PortfolioEvent> saveAll(List<PortfolioEvent> portfolioEvents) {
        var documents = portfolioEvents.stream().map(PortfolioEventDocumentMapper::toDocument).toList();
        try {
            var saved = mongoRepository.saveAll(documents);
            return saved.stream().map(PortfolioEventDocumentMapper::toDomain).toList();
        } catch (DuplicateKeyException e) {
            throw new IllegalStateException("Evento de portfólio duplicado (idempotência)", e);
        }
    }
    ...
    public Optional<PortfolioEvent> findById(String id) {
        return mongoRepository.findById(id).map(PortfolioEventDocumentMapper::toDomain);
    }
}
